...
import h5py
from udnn.model import Model
from udnn.utils import presenter
from udnn.utils.dataloader import DataLoaderNoCI
from udnn.utils.dataloader import Denormalize
    ...
    while i < (((H - sub_size) / stride) + 1) and nev <= stop:
        ...
        if i % valfreq == (valfreq - 1):
            ...
            for m in range(0, leng - 1):
                sys.stdout.write(syss.Blue + '-')
            sys.stdout.write(syss.Blue + 'Evaluation')
            for m in range(0, leng - 1):
                sys.stdout.write(syss.Blue + '-')
            sys.stdout.write(syss.Blue + '\n\n')
            net.eval()
            ...
            print(syss.Cyan + 'Model Saved with Error:' + syss.Green + (
                    ' %1.8f ' % totalError) + syss.Cyan + 'and MSE_CI:' + syss.Green + (' %1.8f \n' % totalErrorCI))
            time = timer.get_value()
            ...
            if nev != 1 and (previousMSE - presentMSE) < 0:
                print(syss.Cyan + 'Changing Learing Rates by 0.1\n')
