...
class DataLoaderUpscalerSynth(Dataset):

    def __init__(self, sino, stride, sub_size, down_scale, scale, W, filename, loc_dataset, size, sinOrder):
        super(DataLoaderUpscalerSynth, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        data = myFile[self.loc_dataset][
               int(index * self.stride):int(index * self.stride + self.sub_size):(int(self.down_scale / self.scale)),
        ...
        Y = torch.Tensor(1, data.shape[0], data.shape[1])
        Y[0, :, :] = torch.from_numpy(data)
        CI = torch.Tensor(1, int((self.sub_size - 1) * ((self.scale - 1) / self.down_scale)), self.W)
        c = 0
        for i in range(0, int((self.sub_size - 1) * (self.scale / self.down_scale) + 1)):
            if i % self.scale != 0:
                CI[0, c, :] = torch.from_numpy(data2[i, :])
                c = c + 1
        return X, CI, Y
...
class DataLoaderUpscaler(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data2 = np.array(
            Image.fromarray(d).resize((self.W, int((self.sub_size - 1) * (self.scale / self.down_scale) + 1)),
                                      Image.BICUBIC))
        d, avg, std = DataLoaderUpscaler.Normalize(d)
        d = d + 0.5
        X = torch.Tensor(1, d.shape[0], d.shape[1])
        X[0, :, :] = torch.from_numpy(d)
        Y = torch.Tensor(1, data.shape[0], data.shape[1])
